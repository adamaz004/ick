...
$result = mysqli_query($link, "SELECT * FROM user_teacher WHERE (username='$user')");
...
if (!$rekord) {
    ...
    if ($rekord["password"] == $pass) {
        ...
        $_SESSION["user"] = $user;
       $_SESSION["user_id"] = $rekord["teacher_id"];
        ...
        header("Location: index2_teacher.php"); //Przejdz dalej do index4.php
    } else {
        wrongPassword();
